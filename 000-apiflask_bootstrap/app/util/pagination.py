...
class PageQueryIn(Schema):
    current = Integer(description='当前页数')
    size = Integer(description='每页大小')


def get_page_query_in(query):
    return query.get('current'), query.get('size')


class PaginationSchemaMixin(object):
    current = Integer()
    size = Integer()
    total = Integer()
